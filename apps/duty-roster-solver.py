...
def _(df_input, df_shifts, mo):
    ...
    number_of_amb_input = mo.ui.number(
        start=1, stop=100, step=1, value=48,
    ...
    # R5 Fairness Slider - with a high default value
    r5_fairness_slider = mo.ui.slider(
        1, 100, value=75, 
        label="Tough Shift Fairness (R5) Priority:"
    )

    # R6 Consistency Slider - with a lower default value
...
def _(
    ...
    # cell 5: Core Solver Logic (Optimized, Fully Commented)
    if (run_button.value and not get_solving() and 
        ...
        try:
            ...
            # R5: Fairness of Tough Shifts (Soft, High Priority)
            _tough_shifts = ['H', 'N']
            _tough_shift_counts = [_model.NewIntVar(0, len(_dates), f'tough_count_{_a}') for _a in _ambulances]
            for _a_idx, _a in enumerate(_ambulances):
                _model.Add(_tough_shift_counts[_a_idx] == sum(_assign[(_d, _a, _s)]
                                                              for _d in _dates for _s in _tough_shifts))

            _total_tough_shifts_to_assign = sum(_demand.get((_d, _s), 0)
                                                for _d in _dates for _s in _tough_shifts)
            if _num_ambulances > 0:
                _min_tough = _total_tough_shifts_to_assign // _num_ambulances
                _max_tough = _min_tough + 1 if _total_tough_shifts_to_assign % _num_ambulances else _min_tough
                # Tighten variable bounds to reduce search space
                for i in range(_num_ambulances):
                    _model.Add(_tough_shift_counts[i] >= _min_tough)
                    _model.Add(_tough_shift_counts[i] <= _max_tough)

            # Add hints
            for _count_var in _tough_shift_counts:
                _model.AddHint(_count_var, _min_tough)

            # Penalize squared deviations for better solver performance
            fairness_sq_devs = []
            for _count_var in _tough_shift_counts:
                _diff = _model.NewIntVar(-_max_tough, _max_tough, f'diff_{_count_var.Name()}')
                _model.Add(_diff == _count_var - _min_tough)
                _sq_dev = _model.NewIntVar(0, _max_tough * _max_tough, f'sq_dev_{_count_var.Name()}')
                _model.AddMultiplicationEquality(_sq_dev, [_diff, _diff])
                fairness_sq_devs.append(_sq_dev)

            # Use a high penalty weight for fairness
            fairness_weight = r5_fairness_slider.value
            _objective_terms.append(fairness_weight * sum(fairness_sq_devs))
            ...
            # Re-enable the button when the process is done.
            set_solving(False)
